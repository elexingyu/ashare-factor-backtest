...
在这项固定策略完整回测中，Qlib 与本项目的墙钟时间比为 **`31.77x`**。日期、因子有限值和每日目标持仓完全一致；逐日净收益最大绝对误差为 `9.29e-08`，总收益误差为 `1.32e-05`，Sharpe 误差为 `1.56e-04`，Rank IC 均值误差为 `5.74e-08`，均通过实验前冻结的容差。

这个倍数只对应上述固定工作负载，不能外推到所有公式、数据规模和机器。合成因子在本次随机市场中亏损也不影响基准目的：它检验的是相同交易结果下的计算正确性和速度，不是展示 alpha。

### A 股完整研究层

本项目自己的生产流程还额外处理 PIT 上市状态、ST、停牌、开盘涨跌停、压力费用、筛选窗口和滚动检验。相同的 500 × 1,500 数据上，一条表达式触发 5 个滚动折、90 次组合模拟和 12 次 Rank IC 计算，完整中位耗时为 **`9.777 s`**，进程峰值内存为 **459 MiB**。

其中中位耗时约为：因子与分块数据读取 `3.860 s`、成交上下文构建 `3.301 s`、滚动检验 `2.033 s`、初筛 `0.580 s`。这里不发布 Qlib 倍数，因为两边没有完全同构的 A 股约束与滚动证据流程；它用于展示真实生产入口的一次完整调用成本。
