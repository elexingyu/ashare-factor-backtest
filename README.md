...
**性能摘要：** 在 500 只证券 × 1,500 个交易日的固定完整单因子回测中，本项目中位耗时 `1.330 s`，Qlib 为 `19.070 s`，约快 **`14.34x`**；速度比仅在 11 项结果一致性检查全部通过后生成。详见[性能证据](#性能证据)。
...
- **收益隔离的因子有效性审计：** `audit-factor` 在不读取收益、目标、IC 或组合结果的前提下，
  ...
  执行自己的 Stage 0 门槛；同时报告每日 breadth、零敞口频率和 net/gross 恒等式，供两阶段
  稳健性审计在解封收益前完成数据门。
...
- **日历日语义明确：** 零暴露交易日以零收益进入主序列；存在非零持仓但持仓价格缺失时仍保留为
  缺失，并通过 active-book 与有限收益日计数显式披露。
...
`audit-factor` 的覆盖分母是任务指定的真实 PIT 股票池单元格，不是完整股票面板。它只报告
可测试性事实，不内置 Alpha、收益或入库门槛。
